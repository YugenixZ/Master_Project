...
SubmodelVars submodel_create(
   ...
   // SCIP_VAR* y_L;
   // SCIP_VAR* y_R;
   // SCIP_CALL_ABORT(SCIPcreateVarBasic(model_sub, &y_L, "y_L", 0.0, 1.0, 0.0, SCIP_VARTYPE_BINARY));
   // SCIP_CALL_ABORT(SCIPaddVar(model_sub, y_L));
   // SCIP_CALL_ABORT(SCIPcreateVarBasic(model_sub, &y_R, "y_R", 0.0, 1.0, 0.0, SCIP_VARTYPE_BINARY));
   // SCIP_CALL_ABORT(SCIPaddVar(model_sub, y_R));
   ...
   // // Link slacks to binaries
   // SCIP_CONS* cons_sL;
   // SCIP_CALL_ABORT(SCIPcreateConsBasicLinear(model_sub, &cons_sL, "force_sl", 0, nullptr, nullptr, 0.0, SCIPinfinity(model_sub)));
   // SCIP_CALL_ABORT(SCIPaddCoefLinear(model_sub, cons_sL, s_L, 1.0));
   // SCIP_CALL_ABORT(SCIPaddCoefLinear(model_sub, cons_sL, y_L, -1e-5)); // epsilon
   // SCIP_CALL_ABORT(SCIPaddCons(model_sub, cons_sL));
   // SCIP_CALL_ABORT(SCIPreleaseCons(model_sub, &cons_sL));

   // SCIP_CONS* cons_sR;
   // SCIP_CALL_ABORT(SCIPcreateConsBasicLinear(model_sub, &cons_sR, "force_sr", 0, nullptr, nullptr, 0.0, SCIPinfinity(model_sub)));
   // SCIP_CALL_ABORT(SCIPaddCoefLinear(model_sub, cons_sR, s_R, 1.0));
   // SCIP_CALL_ABORT(SCIPaddCoefLinear(model_sub, cons_sR, y_R, -1e-5)); // epsilon
   // SCIP_CALL_ABORT(SCIPaddCons(model_sub, cons_sR));
   // SCIP_CALL_ABORT(SCIPreleaseCons(model_sub, &cons_sR));

   // // At least one must be 1
   // SCIP_CONS* cons_y;
   // SCIP_CALL_ABORT(SCIPcreateConsBasicLinear(model_sub, &cons_y, "force_not_both_zero", 0, nullptr, nullptr, 1.0, SCIPinfinity(model_sub)));
   // SCIP_CALL_ABORT(SCIPaddCoefLinear(model_sub, cons_y, y_L, 1.0));
   // SCIP_CALL_ABORT(SCIPaddCoefLinear(model_sub, cons_y, y_R, 1.0));
   // SCIP_CALL_ABORT(SCIPaddCons(model_sub, cons_y));
   // SCIP_CALL_ABORT(SCIPreleaseCons(model_sub, &cons_y));
   ...
   if (condition == "pi0") {
      ...
      for (int i = 0; i < n; ++i) {
         if (SCIPisFeasZero(model_ck, pi_solution[i])) {
            continue; // Skip if pi_solution[i] is zero
         }
      ...
      SCIP_CONS* cons;
      SCIP_Real pi0_plus = pi0_solution + 1;
...
vector<Submodel_sols> submodel_solve(
   ...
   // Initial check on small zl
   SCIP_Real zl_small = zl_low + 1e-5;
   SubmodelVars submodel_small = submodel_create(scip, A, b, c, M, k, delta, zl_small);
   SCIP_RETCODE retcode = SCIPsolve(submodel_small.model_sub);
   if (retcode != SCIP_OKAY) {
   ...
   if (SCIPgetStatus(submodel_small.model_sub) == SCIP_STATUS_OPTIMAL) {
      SCIP_Sol *submodel_sol = SCIPgetBestSol(submodel_small.model_sub);
      vector<SCIP_Real> pi_plus_solution(n);
      vector<SCIP_Real> pi_minus_solution(n);
      vector<SCIP_Real> pi_solution(n);
      for (int i = 0; i < n; ++i) {
         pi_plus_solution[i] = SCIPgetSolVal(submodel_small.model_sub, submodel_sol, submodel_small.pi_plus[i]);
         pi_minus_solution[i] = SCIPgetSolVal(submodel_small.model_sub, submodel_sol, submodel_small.pi_minus[i]);
         pi_solution[i] = pi_plus_solution[i] - pi_minus_solution[i];
      }
      SCIP_Real s_L_solution = SCIPgetSolVal(submodel_small.model_sub, submodel_sol, submodel_small.s_L);
      SCIP_Real s_R_solution = SCIPgetSolVal(submodel_small.model_sub, submodel_sol, submodel_small.s_R);
      SCIP_Real pi0_solution = SCIPgetSolVal(submodel_small.model_sub, submodel_sol, submodel_small.pi0);
      ...
      assert(SCIPisFeasIntegral(submodel_small.model_sub, pi0_solution));
      for (SCIP_Real i : pi_solution) {
         assert(SCIPisFeasIntegral(submodel_small.model_sub, i));
      }
      if (SCIPisFeasZero(scip, s_L_solution) && SCIPisFeasZero(scip, s_R_solution)) {
         // If both slacks are zero, we can skip creating the submodel
         // but if it is at root, then use default branching rule
         SCIP_NODE* curr_node = SCIPgetCurrentNode(scip);
         int node_num = SCIPnodeGetNumber(curr_node);
         if (node_num == 1) {
            
            SCIPreleaseVar(submodel_small.model_sub, &submodel_small.s_L);
            SCIPreleaseVar(submodel_small.model_sub, &submodel_small.s_R);
            SCIPreleaseVar(submodel_small.model_sub, &submodel_small.pi0);
            for (int i = 0; i < m; ++i) {
               SCIPreleaseVar(submodel_small.model_sub, &submodel_small.p[i]);
               SCIPreleaseVar(submodel_small.model_sub, &submodel_small.q[i]);
                  }
            for (int i = 0; i < n; ++i) {
               SCIPreleaseVar(submodel_small.model_sub, &submodel_small.pi_minus[i]);
               SCIPreleaseVar(submodel_small.model_sub, &submodel_small.pi_plus[i]);
               }
            SCIPfree(&submodel_small.model_sub);
            result = {SCIP_INVALID, {}, {}, NULL, NULL, "NULL", "NULL"};
            final_results.push_back(result);
            return final_results;
         }
      ...
      SCIPreleaseVar(submodel_small.model_sub, &submodel_small.s_R);
      SCIPreleaseVar(submodel_small.model_sub, &submodel_small.pi0);
      // SCIPreleaseVar(submodel_small.model_sub, &submodel_small.y_L);
      // SCIPreleaseVar(submodel_small.model_sub, &submodel_small.y_R);
      for (int i = 0; i < m; ++i) {
         SCIPreleaseVar(submodel_small.model_sub, &submodel_small.p[i]);
         SCIPreleaseVar(submodel_small.model_sub, &submodel_small.q[i]);
            }
      ...
      SCIPfree(&submodel_small.model_sub);
      cout << "small model check passed!" << endl;
   }
   else {
      std::cout << "Error solving submodel with small zl: " << zl_small << std::endl;
      result = {SCIP_INVALID, {}, {}, NULL, NULL, "NULL", "NULL"};
      final_results.push_back(result);
      return final_results;
   }
   while (abs(zl_high - zl_low) > 1e-6){
      ...
      if (SCIPgetStatus(submodel_datas.model_sub) == SCIP_STATUS_OPTIMAL) {
         ...
         // //print solution of pi, pi0，curr_zl for debugging 
         // cout << "pi_solution: ";
         // for (int i = 0; i < n; ++i) { 
         //    cout << pi_solution[i] << " ";
         // }  
         ...
         // cout << "curr zl: " << zl << endl;
         // initialize probing mode
         ...
         // Check if pi_solution is not all zero
         if (notallzero(pi_solution)) {
            if (!SCIPisFeasZero(scip, s_L_solution) || !SCIPisFeasZero(scip, s_R_solution)) {
               ...
               if ( !leftinf ) {
                  ...
                  if (SCIPisFeasGT(scip, leftobjval, zl)) {
               ...
               SCIP_CALL_ABORT(SCIPsolveProbingLP(scip, -1, &lperror, &rightinf));
               ...
               if ( lperror ) {
                  cout << "Error in probing right side: " << solstatright << endl;
               ...
               if ( !rightinf ) {
                  ...
                  if (SCIPisFeasGT(scip, rightobjval, zl)) {
               ...
               // SCIP_Real z_l_star_l = createTestModel_2( A, b, c, pi_solution, pi0_solution, TRUE);
               // SCIP_Real z_l_star_r = createTestModel_2( A, b, c, pi_solution, pi0_solution, FALSE);
               // cout << "z_l_star_l: " << z_l_star_l << ", z_l_star_r: " << z_l_star_r << endl;      
               // cout << "pi0_solution: " << pi0_solution << endl;  
...
//            pair<string, SCIP_Real> result_r = check_feasibility(model_ck_r_info.first, model_ck_r_info.second, zl, node_ub);
               ...
               if (result_l.first == "updated_zl" || result_r.first == "updated_zl") {
                  if (feasible_zl.empty() || SCIPisFeasGT(scip, zl, feasible_zl.back()) ){
                     feasible_zl.push(zl);
            ...
            // else if (result_l.first == "obj_val less than zl" && result_r.first == "obj_val less than zl"){
            //    cout << "Both left and right models have objective value less than zl, DEBUG" << endl;
...
//            }
               ...
               SCIPreleaseVar(submodel_datas.model_sub, &submodel_datas.s_R);
               SCIPreleaseVar(submodel_datas.model_sub, &submodel_datas.pi0);
               // SCIPreleaseVar(submodel_datas.model_sub, &submodel_datas.y_L);
               // SCIPreleaseVar(submodel_datas.model_sub, &submodel_datas.y_R);
               for (int i = 0; i < m; ++i) {
                  SCIPreleaseVar(submodel_datas.model_sub, &submodel_datas.p[i]);
                  SCIPreleaseVar(submodel_datas.model_sub, &submodel_datas.q[i]);
               }
...
)
{
   if (side == "left") {
      SCIP_CONS *cons_l;
      SCIP_CALL(SCIPcreateConsLinear(scip, &cons_l, (side + std::to_string(SCIPnodeGetNumber(curr_Node))).c_str(), 0, nullptr, nullptr, -SCIPinfinity(scip), pi0_solution_val, 
      ...
      SCIP_CONS *cons_r;
      SCIP_CALL(SCIPcreateConsLinear(scip, &cons_r, (side + std::to_string(SCIPnodeGetNumber(curr_Node))).c_str(), 0, nullptr, nullptr, pi0_solution_val, SCIPinfinity(scip), 
         TRUE, TRUE, FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, TRUE));
      for (size_t i = 0; i < pi_solution.size(); ++i) {
         if (SCIPisFeasZero(scip, pi_solution[i])) {
            continue;
         }
 ...
 */
void testCSRMatrixTransposeLarge()
{
    // Example: 4x5 matrix
    // [ 1 0 2 0 0 ]
    // [ 0 3 0 4 0 ]
    ...
    A.col_indices = {0, 2, 1, 3, 0, 2, 4, 1, 3};
    A.row_ptr = {0, 2, 4, 7, 9};

    std::cout << "Original matrix (dense):" << std::endl;
    for (int i = 0; i < A.num_rows; ++i) {
        std::vector<double> row(A.num_cols, 0.0);
        for (int j = A.row_ptr[i]; j < A.row_ptr[i+1]; ++j) {
            row[A.col_indices[j]] = A.values[j];
        }
    ...
    std::cout << "Transposed matrix (dense):" << std::endl;
    for (int i = 0; i < At.num_rows; ++i) {
        std::vector<double> row(At.num_cols, 0.0);
        for (int j = At.row_ptr[i]; j < At.row_ptr[i+1]; ++j) {
            row[At.col_indices[j]] = At.values[j];
        }
        for (int j = 0; j < At.num_cols; ++j) {
            std::cout << std::setw(4) << row[j] << " ";
        }
        std::cout << std::endl;
    }
...
SCIP_DECL_BRANCHEXECLP(BranchruleGeneralDisjunction::scip_execlp){
   {
      ...
      size_t m = b.size();
      size_t n = c.size();
      // // Output the current LP relaxation to a .lp file
      // std::string lp_output_path = "/home/htc/yzhou/SCRATCH/current_lp.lp";
      // SCIP_CALL_ABORT(SCIPwriteTransProblem(scip, lp_output_path.c_str(), "lp", FALSE));
      // std::cout << "Current LP relaxation written to: " << lp_output_path << std::endl;
      // // Output the current LP solution to a .sol file (manual write)
      // std::string sol_output_path = "/home/htc/yzhou/SCRATCH/current_lp.sol";
      // std::ofstream solfile(sol_output_path);
      // if (solfile.is_open()) {
      //    SCIP_COL** cols = SCIPgetLPCols(scip);
      //    int ncols = SCIPgetNLPCols(scip);
      //    for (int i = 0; i < ncols; ++i) {
      //    SCIP_VAR* var = SCIPcolGetVar(cols[i]);
      //    SCIP_Real val = SCIPgetSolVal(scip, nullptr, var);
      //    solfile << "x_" << i << " " << val << std::endl;
      //    }
      //    solfile.close();
      //    std::cout << "Current LP solution written to: " << sol_output_path << std::endl;
      // } else {
      //    std::cerr << "Failed to write LP solution to: " << sol_output_path << std::endl;
      // }
      // testCSRMatrixTransposeLarge();
//      testSubmodelCreate();
//      SCIP_LPSOLSTAT status_LP = SCIPgetLPSolstat(scip);
//      if (status_LP == SCIP_LPSOLSTAT_OPTIMAL) {
//         vector<SCIP_Real> x_star(n);
...
//      cout << "Primal solution: " << Primalsol << endl;
      ...
      // for (size_t i = 0; i < vars_lp.size(); ++i)
      // {
      //    SCIP_Real sol_val = SCIPgetSolVal(scip, NULL, vars_lp[i]);
      //    std::cout << "Variable " << i << ": " << sol_val << std::endl;
      //    cout << "Is feasible integer?: " << (SCIPisFeasIntegral(scip, sol_val) ? "Yes" : "No") << std::endl;
      // }
      ...
      if ( status_l == "NULL" || status_r == "NULL") {
         ...
         *result = SCIP_DIDNOTRUN;
         ...
         SCIP_Real pi0_sol_plus = final_results[0].pi0_solution + 1;

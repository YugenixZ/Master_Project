...
void check_vector_SCIP_Real(const std::vector<SCIP_Real>& vec, const std::string& name) {
    int inf_count = 0, nan_count = 0;
    for (size_t i = 0; i < vec.size(); ++i) {
        if (std::isinf(static_cast<double>(vec[i]))) inf_count++;
        if (std::isnan(static_cast<double>(vec[i]))) nan_count++;
    }
    if (inf_count > 0 || nan_count > 0) {
        std::cerr << "Warning: " << name << " contains " << inf_count << " inf and " << nan_count << " NaN values!" << std::endl;
    }
}
...
vector<Submodel_sols> submodel_solve(
   ...
   // check_vector_SCIP_Real(b, "b");
   // check_vector_SCIP_Real(c, "c");
   // check_vector_SCIP_Real(A.values, "A.values");
   // check_vector_SCIP_Real(A.transpose().values, "A.transpose.values");
   ...
   // if (retcode != SCIP_OKAY) {
   //       std::cerr << "Error solving submodel_small: " << std::endl;
   ...
   //       cout << "small model check not passed! Both s_L and s_R are zero!" << endl;
   ...
   while (abs(zl_high - zl_low) > 1e-6){
      ...
      if (SCIPgetStatus(submodel_datas.model_sub) == SCIP_STATUS_OPTIMAL) {
         ...
         if (notallzero(pi_solution)) {
            ...
            // } else {
            //    vector<SCIP_Real> pi_solution_small(n, 0.0);
            //    SCIP_Real pi0_solution_small;
            //    cout << "Both s_L and s_R are zero, DEBUG" << endl;
            //    SubmodelVars submodel_num2 = submodelsmall_create(scip, A, b, c, M, k, delta, zl);
            //    SCIP_CALL_ABORT(SCIPsolve(submodel_num2.model_sub));
            //    SCIP_Sol* submodel_num2_sol = SCIPgetBestSol(submodel_num2.model_sub);
            //    if (SCIPgetStatus(submodel_num2.model_sub) == SCIP_STATUS_OPTIMAL)
            //    {
            //       for (int i = 0; i < n; ++i) {
            //          pi_solution_small[i] = SCIPgetSolVal(submodel_num2.model_sub, submodel_num2_sol, submodel_num2.pi_plus[i]) -
            //                           SCIPgetSolVal(submodel_num2.model_sub, submodel_num2_sol, submodel_num2.pi_minus[i]);
            //       }
            //       pi0_solution_small = SCIPgetSolVal(submodel_num2.model_sub, submodel_num2_sol, submodel_num2.pi0);
            //       SCIP_Real left_obj_test = createTestModel_2(A, b, c, pi_solution_small, pi0_solution_small, TRUE);
            //       SCIP_Real right_obj_test = createTestModel_2(A, b, c, pi_solution_small, pi0_solution_small, FALSE);

            //       cout << "Left obj test: " << left_obj_test << ", pi0: " << pi0_solution_small << endl;
            //       cout << "Right obj test: " << right_obj_test << ", -(pi0 + 1): " << -pi0_solution_small - 1 << endl;

            //    } else {
            //       cout << "Submodel small is infeasible" << endl;                  

            //    }
            //    SCIP_Real left_obj_submodel = createTestModel_2(A, b, c, pi_solution, pi0_solution, TRUE);
            //    SCIP_Real right_obj_submodel = createTestModel_2(A, b, c, pi_solution, pi0_solution, FALSE);

            //    cout << "Left obj submodel: " << left_obj_submodel << ", pi0: " << pi0_solution << endl;
            //    cout << "Right obj submodel: " << right_obj_submodel << ", -(pi0 + 1): " << -pi0_solution - 1 << endl;
            //    zl_high = zl;
            //    continue;
            // }
            if (startprobing) {
            ...
            if (result_l.first == "updated_zl" || result_r.first == "updated_zl") {
               if (feasible_zl.empty() || zl - feasible_zl.back() > 1e-06 || 
               (Status_l.back() == "infeasible" && result_r.first == "infeasible")) {
                  feasible_zl.push(zl);
...
SCIP_DECL_BRANCHEXECLP(BranchruleGeneralDisjunction::scip_execlp){
   {
      ...
      SCIP_LPSOLSTAT status = SCIPgetLPSolstat(scip);
      if (status != SCIP_LPSOLSTAT_OPTIMAL) {
         cout << "LP solution is infeasible, no branching" << endl;
         *result = SCIP_DIDNOTRUN;
         return SCIP_OKAY;
      }
      cout << "LP solution status: " << status << endl;
      ...
      if (fabs(LP_obj - Primalsol) >= 1e-6) {
         std::cerr << "Warning: LP objective and primal solution differ by more than 1e-6. LP_obj = "
             << LP_obj << ", Primalsol = " << Primalsol << std::endl;
      }

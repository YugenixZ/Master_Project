      ...
      )
{
   ...
   if( probname != NULL )
   {
      ...
      if( EPSLT( objlimit, DBL_MAX, MINEPSILON ) )
      {
         ...
         SCIP_CALL_ABORT( SCIPupdateCutoffbound(scip, objlimit) );               
      }
...
// #endif
      ...
      SCIP_PARAM* milp_threads = SCIPgetParam(scip, "presolving/milp/threads");
      if( milp_threads != NULL && SCIPparamGetIntMax(milp_threads) > 1 )
      {
#ifdef _COMM_MPI_WORLD
      ...
      ************************/
      ...
      if( !paraParams->getBoolParamValue(Quiet) && maxrounds != 0 && !qsol )
      {
         os << ".trans";
   ...
   else
   {
      ...
      std::cout << "  -qsol                  : quit to output solution file" << std::endl;

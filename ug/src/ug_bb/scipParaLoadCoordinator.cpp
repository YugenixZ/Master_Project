...
ScipParaLoadCoordinator::~ScipParaLoadCoordinator()
{
   // Calculate fair nodes count before final statistics are output
   // This is done here while SCIP instance in paraInitiator still has valid statistics
   ScipParaInitiator *scipParaInitiator = dynamic_cast<ScipParaInitiator *>(paraInitiator);
   if( scipParaInitiator )
   {
      // The fair nodes count will be calculated using the SCIP instance statistics
      // This is better than calculating it in the Load Coordinator because the 
      // ScipParaInitiator has direct access to the SCIP instance with valid statistics
      long long fairNodes = scipParaInitiator->getFairNodesCount();
      scipParaInitiator->setFairNodesCount(fairNodes);
   }
}
